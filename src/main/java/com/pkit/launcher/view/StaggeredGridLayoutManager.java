...
public class StaggeredGridLayoutManager extends RecyclerView.LayoutManager {
    ...
    public static final String TAG = "StaggeredLayoutManager";
    ...
    private com.pkit.launcher.view.LayoutState mLayoutState;
    ...
    private void checkForGaps() {
        ...
        int invalidGapDir = mShouldReverseLayout ? com.pkit.launcher.view.LayoutState.LAYOUT_START : com.pkit.launcher.view.LayoutState.LAYOUT_END;
    ...
    View hasGapsToFix() {
        ...
        for (int i = firstChildIndex; i != childLimit; i += nextChildDiff) {
            ...
            if (lp.mFullSpan || lp.mHalfFullSpan) {
    ...
    private void ensureOrientationHelper() {
        if (mPrimaryOrientation == null) {
            ...
            mLayoutState = new com.pkit.launcher.view.LayoutState();
        }
    }
        ...
        if (mPendingSavedState == null || mPendingSavedState.mAnchorPosition == NO_POSITION
            ...
            if (child != null) {
                ...
                if (mPendingScrollPositionOffset == INVALID_OFFSET) {
                    ...
                    anchorInfo.mLayoutFromEnd = position == com.pkit.launcher.view.LayoutState.LAYOUT_END;
                    anchorInfo.assignCoordinateFromPadding();
                } else {
    ...
    private void measureChildWithDecorationsAndMargin(View child, LayoutParams lp) {
        ...
        }else if(lp.mHalfFullSpan){
            if (mOrientation == VERTICAL) {
                measureChildWithDecorationsAndMargin(child, mFullSizeSpec/2, mHeightSpec);
            } else {
                measureChildWithDecorationsAndMargin(child, mWidthSpec, mFullSizeSpec/2);
            }
        } else {
    ...
    public void onInitializeAccessibilityNodeInfoForItem(RecyclerView.Recycler recycler,
        ...
        if (mOrientation == HORIZONTAL) {
            info.setCollectionItemInfo(AccessibilityNodeInfoCompat.CollectionItemInfoCompat.obtain(
                    sglp.getSpanIndex(), sglp.mFullSpan ? (sglp.mHalfFullSpan ? mSpanCount : 2) : 1,
                    -1, -1,
    ...
    private void updateLayoutStateToFillStart(int anchorPosition, RecyclerView.State state) {
        ...
        mLayoutState.mLayoutDirection = com.pkit.launcher.view.LayoutState.LAYOUT_START;
        mLayoutState.mItemDirection = mShouldReverseLayout ? com.pkit.launcher.view.LayoutState.ITEM_DIRECTION_TAIL
                : com.pkit.launcher.view.LayoutState.ITEM_DIRECTION_HEAD;
    }
    ...
    private void updateLayoutStateToFillEnd(int anchorPosition, RecyclerView.State state) {
        ...
        mLayoutState.mLayoutDirection = com.pkit.launcher.view.LayoutState.LAYOUT_END;
        mLayoutState.mItemDirection = mShouldReverseLayout ? com.pkit.launcher.view.LayoutState.ITEM_DIRECTION_HEAD
                : com.pkit.launcher.view.LayoutState.ITEM_DIRECTION_TAIL;
    }
    ...
    private int fill(RecyclerView.Recycler recycler, com.pkit.launcher.view.LayoutState layoutState,
            RecyclerView.State state) {
        ...
        */
        ...
        if (layoutState.mLayoutDirection == com.pkit.launcher.view.LayoutState.LAYOUT_END) {
        ...
        while (layoutState.hasMore(state) && !mRemainingSpans.isEmpty()) {
            ...
            if (layoutState.mLayoutDirection == com.pkit.launcher.view.LayoutState.LAYOUT_END) {
                ...
                if (assignSpan && lp.mFullSpan) {
                    ...
                    fullSpanItem.mGapDir = com.pkit.launcher.view.LayoutState.LAYOUT_START;
                ...
                if (assignSpan && lp.mFullSpan) {
                    ...
                    fullSpanItem.mGapDir = com.pkit.launcher.view.LayoutState.LAYOUT_END;
                    fullSpanItem.mPosition = position;
                    mLazySpanLookup.addFullSpanItem(fullSpanItem);
                }
            }
            ...
            if (lp.mFullSpan && layoutState.mItemDirection == com.pkit.launcher.view.LayoutState.ITEM_DIRECTION_HEAD && assignSpan) {
        ...
        if (mLayoutState.mLayoutDirection == com.pkit.launcher.view.LayoutState.LAYOUT_START) {
    ...
    private void attachViewToSpans(View view, LayoutParams lp, com.pkit.launcher.view.LayoutState layoutState) {
        if (layoutState.mLayoutDirection == com.pkit.launcher.view.LayoutState.LAYOUT_END) {
            if (lp.mFullSpan) {
    ...
    private void recycle(RecyclerView.Recycler recycler, com.pkit.launcher.view.LayoutState layoutState,
            Span updatedSpan, int recycleLine) {
        if (layoutState.mLayoutDirection == com.pkit.launcher.view.LayoutState.LAYOUT_START) {
    ...
    private void updateRemainingSpans(Span span, int layoutDir, int targetLine) {
        ...
        if (layoutDir == com.pkit.launcher.view.LayoutState.LAYOUT_START) {
    ...
    private boolean preferLastSpan(int layoutDir) {
        if (mOrientation == HORIZONTAL) {
            return (layoutDir == com.pkit.launcher.view.LayoutState.LAYOUT_START) != mShouldReverseLayout;
        }
        return ((layoutDir == com.pkit.launcher.view.LayoutState.LAYOUT_START) == mShouldReverseLayout) == isLayoutRTL();
    }

    /**
     * Finds the span for the next view.
     */
    private Span getNextSpan(com.pkit.launcher.view.LayoutState layoutState) {
    ...
    private int calculateScrollDirectionForPosition(int position) {
        if (getChildCount() == 0) {
            return mShouldReverseLayout ? com.pkit.launcher.view.LayoutState.LAYOUT_END : com.pkit.launcher.view.LayoutState.LAYOUT_START;
        }
        final int firstChildPos = getFirstChildPosition();
        return position < firstChildPos != mShouldReverseLayout ? com.pkit.launcher.view.LayoutState.LAYOUT_START : com.pkit.launcher.view.LayoutState.LAYOUT_END;
    }
    ...
    public void smoothScrollToPosition(final RecyclerView recyclerView, final int position) {
        LinearSmoothScroller scroller = new LinearSmoothScroller(recyclerView.getContext()) {
            @Override
            public PointF computeScrollVectorForPosition(int targetPosition) {
                if (getChildCount() == 0) {
            ...
            protected void onTargetFound(View targetView, RecyclerView.State state, Action action) {
                ...
                if (time > 0) {
                    if(position == 12){
                        dx -= recyclerView.getContext().getResources().getDimension(R.dimen.staggered_dimen);
                    }
     ...
     */
    public int findFirstReferenceChildPosition(int itemCount) {
        final int limit = getChildCount();
        for (int i = 0; i < limit; i++) {
     ...
     * @return Position of the View or 0 if it cannot find any such View.
     */
    public int findLastReferenceChildPosition(int itemCount) {
        for (int i = getChildCount() - 1; i >= 0; i--) {
            final View view = getChildAt(i);
            final int position = getPosition(view);
            if (position >= 0 && position < itemCount) {
    ...
    public static class LayoutParams extends RecyclerView.LayoutParams {
        ...
        public void setHalfFullSpan(boolean halfFullSpan){
            mHalfFullSpan = halfFullSpan;
        }

        public boolean isHalfFullSpan() {
            return mHalfFullSpan;
        }
    ...
    class Span {
        ...
        void calculateCachedStart() {
            ...
            if (lp.mFullSpan) {
                ...
                if (fsi != null && fsi.mGapDir == com.pkit.launcher.view.LayoutState.LAYOUT_START) {
        ...
        void calculateCachedEnd() {
            ...
            if (lp.mFullSpan) {
                LazySpanLookup.FullSpanItem fsi = mLazySpanLookup
                        .getFullSpanItem(lp.getViewPosition());
                if (fsi != null && fsi.mGapDir == com.pkit.launcher.view.LayoutState.LAYOUT_END) {
    ...
    private class AnchorInfo {
        ...
        public String toString() {
            return "{mPosition:"+mPosition+
                    ...
                    ",mInvalidateOffsets:"+mInvalidateOffsets+"}";
        }
    }
}
